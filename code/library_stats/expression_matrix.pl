...
foreach my $gene (@genes) {
	...
	foreach my $sample (@sample_names) {
		...
		die "No reads mapped to miRNAs found in sample $sample. Perhaps the mirna_species.txt file is empty?\n" if $samples{$sample} == 0;
		my $norm_count = sprintf("%.6f", $count*$norm_factor/$samples{$sample});
		my $log_count = sprintf("%.6f", $norm_count == 0 ? 0 : log($norm_count) / log(2));
